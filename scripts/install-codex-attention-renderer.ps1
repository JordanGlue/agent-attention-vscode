...
$cssTag = "`t`t<link rel=`"stylesheet`" href=`"./codex-attention-renderer.css?v=0.3.0`">"
$jsTag = "`t<script src=`"./codex-attention-renderer.js?v=0.3.0`" type=`"module`"></script>"
$html = Get-Content -Raw -LiteralPath $htmlPath

if ($Remove) {
    $updated = $html.Replace("$cssTag`r`n", '').Replace("$cssTag`n", '')
    $updated = $updated.Replace("$jsTag`r`n", '').Replace("$jsTag`n", '')
    if ($updated -ne $html) {
    ...
    Write-Host "Removed Codex pane attention renderer from VS Code build $($build.Name)."
...
$backupDir = Join-Path $HOME ".codex\backups\vscode-renderer\$($build.Name)"
...
Write-Host "Installed Codex pane attention renderer into VS Code build $($build.Name)."
